...
def test_create_user_direct(db):
    ...
    result = user_controller.create_user(user, db)
    assert result.email == user.email
    assert result.id is not None

def test_get_user_direct(db):
    user = UserCreate(name="Carlos", email="carlos@example.com", password="1234")
    created = user_controller.create_user(user, db)
    fetched = user_controller.get_user(created.id, db)
